...
RSpec.describe 'New viewing party page' do
  ...
  it "can see a form with the following details", :vcr do
    ...
    expect(PartyViewing.count).to eq(1)
    expect(PartyViewing.first.movie_title).to eq(@movie.title)
    expect(PartyViewing.first.movie_id).to eq(@movie.id)
    expect(PartyViewing.first.runtime).to eq(@movie.runtime)
    expect(PartyViewing.first.date).to eq("10/13/2020")
    expect(PartyViewing.first.time).to eq("4:00")
    expect(PartyViewing.first.user).to eq(@drewni)
    expect(PartyViewing.first.guests).to eq([@hank, @homer])
